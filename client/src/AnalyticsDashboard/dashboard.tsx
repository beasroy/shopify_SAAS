...
export default function Dashboard() {
  ...
  const fetchAdData = useCallback(async () => {
    ...
    try {
      ...
      const googleData = googleAdResponse.data.data;
  ...
  const calculateAggregatedMetrics = (fbData: AdAccountData[], googleData: GoogleAdAccountData | undefined) => {
    ...
    setAggregatedMetrics({
      ...
      totalROAS: (totalRevenue / totalSpent).toFixed(2),
      totalPurchases:totalPurchases.toLocaleString(),
      totalCTR:`${((totalClicks / totalImpressions)*100).toFixed(2)} %`,
      totalCPC: (totalSpent/totalClicks).toFixed(2),
      // Add other aggregated fields as needed, like CPC, CTR, etc.
    });
  };
  ...
  const metrics = [
    ...
    },
    {
      ...
      tooltipContent: 'Total amount spent'
    },
    {
      label: 'ROAS (Ads only)',
      ...
      <main className="p-4 md:p-6 lg:p-8">
        <div className="space-y-6">
          ...
          <section>
            ...
            <AdAccountMetricsCard metrics={metrics} date={date || { from: new Date(), to: new Date() }} isLoading={isLoading} icon='' />
          </section>
        </div>


        {fbAdAccountsMetrics.length > 0 && fbAdAccountsMetrics.map((accountMetrics, index) => {
          // Construct the metrics array for each account
          const fbmetrics = [
