...
import {
  ...
  setBrands,
  resetBrand,
} from "@/store/slices/BrandSlice.ts";
...
import { removeBrandName } from "@/store/slices/BrandNameSlice.ts";
...
export default function CollapsibleSidebar() {
  ...
  const handleLogout = async () => {
    try {
      ...
      if (response.status === 200) {
        ...
        // Remove brand name from redux store.
        dispatch(removeBrandName(true));
      }
    } catch (error) {
  ...
  const allDashboards = [
    ...
    {
      name: "Marketing Insights Tracker",
      path: `/marketing-insights/${selectedBrandId}`,
      icon: <CalendarRange size={20} />,
    },
    {
      name: "Ad Metrics Hub",
      path: `/admetrics/${selectedBrandId}`,
      ...
      subItems: [
        {
          ...
          path: `/ecommerce-reports/${selectedBrandId}`,
        },
        {
          name: "Conversion Lens",
          path: `/conversion-reports/${selectedBrandId}`,
        },
      ],
    },
    {
  ...
  const isItemDisabled = (item: any) => {
    ...
    if (
      (item.name === "Analytics Dashboard" ||
        item.name === "Campaign Metrics") &&
  ...
  return (
    <TooltipProvider>
      <div
        ref={sidebarRef}
        className={`bg-[rgb(4,16,33)] text-white transition-all duration-500 ease-in-out flex flex-col ${
      ...
      >
        ...
        <div
          className={`flex-1 overflow-y-auto ${
            isExpanded ? "h-[calc(100vh-64px)]" : "h-[calc(100vh-16px)]"
          }`}
        >
            ...
            <nav className="mt-3">
              ...
              <SidebarItem
                ...
                text={
                  selectedBrandId
                    ? brands
                        .find((b) => b._id === selectedBrandId)
                        ?.name.replace(/_/g, " ") || "Unknown Brand"
                    : "Your Brands"
                }
              ...
              {allDashboards.map((item, index) => {
                const isAnySubItemSelected = item.subItems?.some(
                ...
                const dashboardItemContent = (
                  <div
                    className={`flex items-center px-4 py-1.5 mb-1 text-gray-300 hover:bg-gray-700 hover:text-white transition-colors duration-200 ${
                      item.subItems ? "cursor-auto" : "cursor-pointer"
                    }
                      ${
                        isMainItemActive
                      ...
                      <div className="pl-8 mb-2">
                        ...
                        {item.subItems.map((subItem, subIndex) => {
                          ...
                          return (
                            <div
                              key={subIndex}
                              className={`flex items-center text-xs w-full p-2 transition-colors duration-200 ${
                                disabled
                                  ? "opacity-50"
...
function SidebarItem({
  ...
  const content = (
    ...
    >
      ...
      {isExpanded && (
        <span className="ml-auto">{isOpen ? openIcon : closeIcon}</span>
      )}
    </div>
  );
...
function UserProfile({ isExpanded, user }: UserProfileProps) {
  ...
  const userProfileContent = (
    ...
    >
      ...
      {isExpanded && (
        <span className="text-sm mr-2">{user?.username || "user"}</span>
      )}
    </div>
  );
