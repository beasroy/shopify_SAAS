...
export default function MarketingInsightsCard({
  onNavigate,
  brandId,
  startDate,
  endDate,
    ...
    const fetchMetrics = async () => {
      try {
        ...
        const response = await axiosInstance.get(
          `/api/highlights/marketing/${brandId}`,
          {
            params: {
              ...(startDate ? { startDate } : {}),
              ...(endDate ? { endDate } : {}),
  ...
  const getColumns = () => {
    ...
    return Object.keys(metrics[0]).filter(
          ...
          <table className="w-full text-xs">
            <thead className="sticky top-0 bg-white z-10">
              <tr className="border-b border-slate-200">
                <th className="text-left p-2 font-semibold text-slate-600 sticky left-0 bg-white">
                  Period
                </th>
                {columns.map((column) => (
                  <th
                    key={column}
                    className="text-right p-2 font-semibold text-slate-600 whitespace-nowrap"
                  >
                  ...
                  {columns.map((column) => (
                    <td
                      ...
                      className={cn(
                        ...
                        getCellColorClass(
                          column,
                          metric[column as keyof MonthlyMetric] as number,
                        ),
                      )}
                    >
                      {formatCellValue(
