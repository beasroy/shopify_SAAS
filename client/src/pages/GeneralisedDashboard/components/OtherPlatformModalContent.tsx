...
export default function OtherPlatformModalContent({
      ...
      try {
        ...
        if (response.status === 200) {
          ...
          setGoogleAdsAccounts(
            response.data?.clientAccounts.filter(
              (account: GoogleAdsAccount) => !account.hidden
            )
          );
        } else {
    ...
    if (axios.isAxiosError(error)) {
      ...
      if (response) {
        ...
        if (data?.errors && data.errors.length > 0) {
          ...
          const googleAdsErrorMessage =
            googleAdsError.message || "Google Ads API Error";
  ...
  const handleConnect = (
    account: GoogleAdsAccount | GoogleAnalyticsAccount | FacebookAdsAccount
  ) => {
  ...
  const handleGoogleAdLogin = async () => {
    const state = `googleAdSetup|${platform}`;

    try {
      const response = await axios.get(
        `${baseURL}/api/auth/google?context=${state}`,
        { withCredentials: true }
      );
  ...
  const handleGoogleAnalyticsLogin = async () => {
    const state = `googleAnalyticsSetup|${platform}`;
                ...
                >
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <Button
                      ...
                      className={
                        connectedId.includes(accountId)
                          ? "bg-green-600"
                          : "bg-blue-600"
                      }
                    >
