...
export default function PerformanceTable({
  ...
  const metricLabels: Record<string, string> = {
    ...
    googlecpm: "Google CPM",
    googlectr: "Google CTR",
    googlecpp: "Google CPP",
  ...
  const getPlatformLogo = (metricKey: string) => {
    if (metricKey.startsWith("meta"))
      return <FacebookLogo width={"1rem"} height={"1rem"} />;
    if (metricKey.startsWith("google"))
  ...
  const getMetricData = (
    ...
    // Check analytics
    if (
      performanceData.analytics?.[period]?.[metricKey] &&
      apiStatus.analytics
    ) {
  ...
  const formatMetricValue = (value: number, metricKey: string) => {
    ...
    if (
      lowerKey.includes("roas") ||
      lowerKey.includes("rate") ||
      lowerKey.includes("cpc") ||
      lowerKey.includes("cpm") ||
      lowerKey.includes("ctr") ||
      lowerKey.includes("cpp")
    ) {
                ...
                {periods.map((period) => {
                  ...
                  if (!data) {
                    return (
                      <td
                        key={period}
                        className="p-3 text-center text-slate-400 text-sm"
                      >
