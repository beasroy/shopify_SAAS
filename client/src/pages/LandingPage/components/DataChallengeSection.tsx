      ...
      <div className="max-w-6xl mx-auto px-6">
        <FadeIn className="text-center mb-16">
          ...
          <h2 className="text-4xl font-extrabold text-slate-900 tracking-tight mb-4">
            ...
            <span className="gradient-text">Your decisions shouldn't be.</span>
          </h2>
          <p className="text-lg text-slate-500 max-w-xl mx-auto">
            Every platform tells a different story. Without a unified view, you're making six-figure decisions in the dark.
          </p>
        </FadeIn>
        ...
        <div className="flex flex-col lg:flex-row items-center gap-6 justify-center">
            ...
            {SILOS.map((silo, i) => (
              <FadeIn key={silo.name} delay={i * 0.1} direction="left">
                <div className="bg-white border border-slate-200 rounded-xl p-4 flex items-start gap-3 shadow-sm w-full lg:w-64">
                  ...
                  <div>
                    <p className="text-sm font-semibold text-slate-800">{silo.name}</p>
                    <p className="text-xs text-slate-500 mt-0.5">{silo.problem}</p>
                  </div>
          ...
          <FadeIn delay={0.3} direction="none" className="flex items-center justify-center">
            <div className="flex flex-col items-center gap-2 text-slate-400">
              ...
              <div className="bg-indigo-600 rounded-full p-3 shadow-lg shadow-indigo-200">
                <ArrowRight size={20} className="text-white lg:hidden" />
                <ArrowRight size={20} className="text-white hidden lg:block rotate-0" />
              </div>
          ...
          <FadeIn delay={0.45} direction="right">
            <div className="bg-white border-2 border-indigo-200 rounded-2xl p-6 shadow-xl shadow-indigo-100 w-full lg:w-72">
              <div className="flex items-center gap-3 mb-4">
                ...
                <div>
                  <p className="text-sm font-bold text-slate-900">Parallels</p>
                  <p className="text-xs text-indigo-600">Unified source of truth</p>
                </div>
              </div>
              <div className="space-y-2">
                {['Cross-channel ROAS', 'Revenue attribution', 'Unified funnel', 'Real-time sync'].map((item) => (
