    ...
    <section className="py-20 bg-gradient-to-br from-indigo-600 via-indigo-700 to-cyan-600 relative overflow-hidden">
      <div className="relative max-w-6xl mx-auto px-6">
        <FadeIn className="text-center mb-12">
          ...
          <h2 className="text-3xl font-extrabold text-white tracking-tight">Results that speak for themselves</h2>
        </FadeIn>
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-8">
          {STATS.map((stat, i) => (
            <motion.div key={stat.label} initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: i * 0.1 }} className="text-center">
              <div className="text-4xl lg:text-5xl font-extrabold text-white mb-1"><CountUp end={stat.value} suffix={stat.suffix} duration={1800} /></div>
