  ...
  return (
    <motion.header
      ...
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
        scrolled ? 'bg-white/94 backdrop-blur-xl shadow-sm border-b border-slate-200/70' : 'bg-transparent'
      }`}
    >
      <div className="max-w-6xl mx-auto px-6 h-16 flex items-center justify-between">
        ...
        <div className="hidden md:flex items-center gap-3">
          ...
          <Link to="/login" className="btn-primary inline-flex items-center gap-1.5 text-white text-sm font-semibold px-4 py-2 rounded-lg">
            Get Demo
          </Link>
        </div>
            ...
            <div className="px-6 py-4 flex flex-col gap-4">
              {NAV_LINKS.map(link => (
                <a key={link.label} href={link.href} onClick={()=>setMenuOpen(false)}
                  className="text-sm font-medium text-slate-700">{link.label}</a>
              ))}
              <div className="flex flex-col gap-2 pt-2 border-t border-slate-100">
                <Link to="/login" className="text-sm font-medium text-slate-700 py-2">Sign In</Link>
                <Link to="/login" className="btn-primary text-white text-sm font-semibold px-4 py-2 rounded-lg text-center">
                  Get Demo
                </Link>
              </div>
            </div>
