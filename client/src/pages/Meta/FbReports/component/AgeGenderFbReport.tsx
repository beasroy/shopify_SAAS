...
const AgeGenderFbReport: React.FC<CityBasedReportsProps> = ({
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const response = await axiosInstance.post(
        `/api/meta/report/age-gender/${brandId}`,
        {
          startDate: startDate,
          endDate: endDate,
        },
  ...
  const primaryColumn = "age_gender";
  ...
  const blendedAgeGenderData = apiResponse?.blendedAgeGenderData;

  useEffect(() => {
    if (apiResponse?.data && !selectedAccount) {
      if (blendedAgeGenderData && blendedAgeGenderData.length > 0) {
  ...
  }, [apiResponse, selectedAccount, blendedAgeGenderData]);

  const currentData = useMemo(() => {
    if (selectedAccount === "blended-summary") {
      return blendedAgeGenderData || [];
    }
    const account = apiResponse?.data.find(
      (acc) => acc.account_name === selectedAccount,
    );
    return account?.ageGenderData || [];
  }, [selectedAccount, apiResponse, blendedAgeGenderData]);
  ...
  if (
    ...
    (!blendedAgeGenderData || blendedAgeGenderData.length === 0)
  ) {
    const account = apiResponse.data[0];
        ...
        >
          ...
          <CardContent className="p-0">
            <div className="rounded-b-lg overflow-hidden px-2.5 pb-2.5">
              <MetaReportTable
                data={
                  Array.isArray(account.ageGenderData)
                    ? account.ageGenderData
                    : [account.ageGenderData]
                }
                primaryColumn={primaryColumn}
...
export default AgeGenderFbReport;
