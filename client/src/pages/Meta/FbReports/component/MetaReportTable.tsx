...
export default function MetaReportTable({
  ...
  const thresholds = useMemo(() => {
    ...
    data.forEach((row) => {
      ...
      if (Array.isArray(monthlyData)) {
        monthlyData.forEach((month) => {
          if (
            typeof month["Spend"] === "number" &&
            typeof month["Purchase ROAS"] === "number"
          ) {
  ...
  const handleRowsPerPageChange = (value: string) => {
    const newRowsPerPage =
      value === "all" ? filteredData.length : parseInt(value);
    setRowsPerPage(newRowsPerPage);
  };

  const renderCell = (
    value: number | string,
    type?: "spend" | "percentage" | "default" | "sessions",
  ) => {
      ...
      <td className="text-right whitespace-nowrap p-3 text-sm border-r border-b border-gray-200 bg-transparent">
        <div className="space-y-1">
          <div className="font-medium truncate">
            {renderCell(spend, "spend")}
          </div>
          <div className="text-xs truncate">
            {renderCell(purchaseROAS, "percentage")}
          </div>
          {purchaseConversionValue !== undefined && (
            <div className="text-xs truncate">
              PCV: {purchaseConversionValue.toLocaleString(locale)}
            </div>
          )}
        </div>
      </td>
  ...
  const renderSummaryCell = (
      ...
      return (
        <td
          className={`p-3 text-sm border-r border-b border-gray-200 ${
            columnIndex < 3
              ? "sticky z-10 bg-white group-hover:bg-blue-50"
              : "bg-transparent"
          }`}
          style={
            columnIndex < 3 ? { left: `${getLeftPosition(columnIndex)}px` } : {}
          }
        >
          {""}
        </td>
    ...
    const totalPCV =
      typeof row["Total PCV"] === "number"
        ? row["Total PCV"].toLocaleString(locale)
        : null;
    ...
    if (column.includes("Purchase ROAS")) {
      return (
        ...
        >
          <div className="text-right">
            <div className="font-medium truncate">
              {renderCell(value, "percentage")}
            </div>
            {totalPCV !== null && (
              <div className="text-xs mt-1 truncate">Total PCV: {totalPCV}</div>
            )}
          </div>
        </td>
      );
      ...
      >
        <td className="sticky left-0 min-w-[130px] max-w-[200px] p-3 text-sm font-medium z-10 bg-white group-hover:bg-blue-50 border-r border-b border-gray-200">
          <div className="truncate">
            {typeof row[primaryColumn] === "string" ||
            typeof row[primaryColumn] === "number"
        ...
        {secondaryColumns?.map((column, columnIndex) => {
          ...
          return (
            <React.Fragment key={column}>
              {renderSummaryCell(row, column, absoluteIndex)}
            </React.Fragment>
          );
        })}
        {months.map((month) => {
    ...
    return (
      <th
        ...
        className={`sticky top-0 z-10 px-3 py-3 text-right text-sm font-medium bg-gray-100 border-r border-b border-gray-200 ${
          isFirst
            ? `z-20 ${columnIndex === 2 ? "shadow-[4px_0_5px_0_rgba(0,0,0,0.09)]" : ""}`
            : ""
      ...
      >
        <div className="flex flex-col">
          ...
          {thresholdValue && (
            <span className="text-xs font-normal mt-0.5 truncate">
              {thresholdValue}
            </span>
          )}
        </div>
      ...
      >
        <table className="w-full table-fixed border border-gray-200">
          <colgroup>
            {safeWidths.map((w, i) => (
              <col
                key={String(columns[i]?.key || `col-${i}`)}
                style={{ width: w }}
              />
            ))}
          </colgroup>
          <thead>
            <tr>
              <th className="sticky left-0 top-0 min-w-[130px] z-30 px-3 py-3 text-left text-sm font-medium bg-gray-100 border-r border-b border-gray-200">
                <div className="flex items-center gap-2">
                  ...
                  {filter && filter.length > 0 && (
                    <span className="text-xs px-2 py-1 rounded">Filtered</span>
                  )}
                </div>
              ...
              {months.map((month, monthIndex) => {
                ...
                const columnIndex =
                  1 + (secondaryColumns?.length || 0) + monthIndex;
                const active = resizingIndex === columnIndex;
                ...
                return (
                  ...
                  >
                    ...
                    {primaryColumn === "All Page" ? "Bounce Rate" : "Spend / Purchase ROAS"}
                  </div>
            ...
            <div className="text-sm">
              {totalRows > 0 ? (
                ...
                <>
                  ...
                  {filter !== undefined ? (
                    <span className="ml-2 text-xs">
                      (Filter: {filter?.join(", ") || "Unknown"}) | Total rows:{" "}
                      {data.length}
                    </span>
                  ) : null}
                </>
