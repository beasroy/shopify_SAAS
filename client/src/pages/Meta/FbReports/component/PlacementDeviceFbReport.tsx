...
type ApiResponse = {
  data: Array<{
    account_name: string;
    placementDeviceData: Array<{
      placement_device: string;
      "Total Spend": number;
  ...
  blendedPlacementDeviceData: Array<{
    placement_device: string;
    "Total Spend": number;
    "Total Purchase ROAS": number;
    "Total PCV": number;
...
const PlacementDeviceFbReport: React.FC<CityBasedReportsProps> = ({
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const response = await axiosInstance.post(
        `/api/meta/report/placement-device/${brandId}`,
        {
          startDate: startDate,
          endDate: endDate,
        },
  ...
  const primaryColumn = "placement_device";
  ...
  const blendedPlacementDeviceData = apiResponse?.blendedPlacementDeviceData;

  useEffect(() => {
    if (apiResponse?.data && !selectedAccount) {
      if (blendedPlacementDeviceData && blendedPlacementDeviceData.length > 0) {
  ...
  }, [apiResponse, selectedAccount, blendedPlacementDeviceData]);

  const currentData = useMemo(() => {
    if (selectedAccount === "blended-summary") {
      return blendedPlacementDeviceData || [];
    }
    const account = apiResponse?.data.find(
      (acc) => acc.account_name === selectedAccount,
    );
    return account?.placementDeviceData || [];
  }, [selectedAccount, apiResponse, blendedPlacementDeviceData]);
  ...
  if (
    ...
    (!blendedPlacementDeviceData || blendedPlacementDeviceData.length === 0)
  ) {
    const account = apiResponse.data[0];
        ...
        >
          <div className="bg-white rounded-md pt-2 px-3">
            <div className="flex items-center space-x-2">
              <PerformanceSummary
                ...
                data={account.placementDeviceData || []}
          ...
          <CardContent className="p-0">
            <div className="rounded-b-lg overflow-hidden px-2.5 pb-2.5">
              <MetaReportTable
                data={
                  Array.isArray(account.placementDeviceData)
                    ? account.placementDeviceData
                    : [account.placementDeviceData]
                }
                primaryColumn={primaryColumn}
            ...
            <div className="flex items-center gap-2">
              <DropdownMenu>
                ...
                <DropdownMenuContent align="end" className="w-48">
                  {blendedPlacementDeviceData &&
                    blendedPlacementDeviceData.length > 0 && (
                      <DropdownMenuItem
...
export default PlacementDeviceFbReport;
