...
const allColumns: ColumnDef[] = [
  ...
  { key: "returnedCustomers", header: "Returned Customers", width: 150, minWidth: 130, align: "right" },
]
...
function getColumnLogo(key: keyof FunnelRow): React.ReactNode | null {
  ...
  if (key === 'aov' || key === 'averageItemsPerOrder' || key === 'codOrderCount' || key === 'prepaidOrderCount' || key === 'productsLaunched' || key === 'returnedCustomers') {
...
export default function ReportTable({
  ...
  const columns = React.useMemo(() => {
    ...
    const columnNameToKey: Record<string, keyof FunnelRow> = {
      ...
      'Returned Customers': 'returnedCustomers'
    };
