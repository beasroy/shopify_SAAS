...
export async function getAnalyticsSummary(req, res) {
  try {
    ...
    const [
      ...
      fetchAnalyticsData(thisMonth, today),
      fetchAnalyticsData(lastMonth, lastMonthEnd)
    ]);
    ...
    const summaries = {
      ...
      monthly: {
        ...
        sessions: {
          current: thisMonthMetrics.sessions,
          previous: lastMonthMetrics.sessions,
          change: getPercentageChange(thisMonthMetrics.sessions, lastMonthMetrics.sessions),
          trend: thisMonthMetrics.sessions >= lastMonthMetrics.sessions ? 'up' : 'down'
        },
        addToCarts: {
          current: thisMonthMetrics.addToCarts,
          previous: lastMonthMetrics.addToCarts,
          change: getPercentageChange(thisMonthMetrics.addToCarts, lastMonthMetrics.addToCarts),
          trend: thisMonthMetrics.addToCarts >= lastMonthMetrics.addToCarts ? 'up' : 'down'
        },
        checkouts: {
          current: thisMonthMetrics.checkouts,
          previous: lastMonthMetrics.checkouts,
          change: getPercentageChange(thisMonthMetrics.checkouts, lastMonthMetrics.checkouts),
          trend: thisMonthMetrics.checkouts >= lastMonthMetrics.checkouts ? 'up' : 'down'
        },
        purchases: {
          current: thisMonthMetrics.purchases,
          previous: lastMonthMetrics.purchases,
          change: getPercentageChange(thisMonthMetrics.purchases, lastMonthMetrics.purchases),
          trend: thisMonthMetrics.purchases >= lastMonthMetrics.purchases ? 'up' : 'down'
        }
      }
    };
    ...
    // Add daily highlights
    Object.entries(summaries.daily).forEach(([metric, data]) => {
      if (typeof data === 'object' && Math.abs(data.change) >= 10) {
        highlights.push({
          metric,
          period: 'daily',
          message: `${metric} ${data.trend === 'up' ? 'increased' : 'decreased'} by ${Math.abs(data.change)}% compared to yesterday`
        });
      }
    });

    // Add weekly highlights
    Object.entries(summaries.weekly).forEach(([metric, data]) => {
      if (typeof data === 'object' && Math.abs(data.change) >= 12) {
        highlights.push({
          metric,
          period: 'weekly',
          message: `${metric} ${data.trend === 'up' ? 'increased' : 'decreased'} by ${Math.abs(data.change)}% compared to previous week`
        });
      }
    });

    // Add monthly highlights
    Object.entries(summaries.monthly).forEach(([metric, data]) => {
      if (typeof data === 'object' && Math.abs(data.change) >= 15) {
        highlights.push({
          metric,
          period: 'monthly',
          message: `${metric} ${data.trend === 'up' ? 'increased' : 'decreased'} by ${Math.abs(data.change)}% compared to last month`
        });
      }
    });
