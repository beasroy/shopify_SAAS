...
const calculateBlendedAgeSummary = (accountsData) => {
  ...
  const blendedSummary = Array.from(blendedData.values())
    .map((age) => ({
      ...age,
      "Total Purchase ROAS":
        age["Total Spend"] > 0 ? age["Total PCV"] / age["Total Spend"] : 0,
      ...
      try {
        ...
        const formattedAges = Array.from(allData.values())
          .map((age) => ({
            ...
            "Total Purchase ROAS":
              age["Total Spend"] > 0
                ? age["Total PCV"] / age["Total Spend"]
                : 0,
            MonthlyData: Array.from(age.MonthlyData.values()).sort((a, b) =>
...
const calculateBlendedGenderSummary = (accountsData) => {
  ...
  const blendedSummary = Array.from(blendedData.values())
    .map((gender) => ({
      ...gender,
      "Total Purchase ROAS":
        gender["Total Spend"] > 0
          ? gender["Total PCV"] / gender["Total Spend"]
          : 0,
...
export const fetchFbGenderReports = async (req, res) => {
  ...
  try {
          ...
          .map((gender) => ({
            ...
            "Total Purchase ROAS":
              gender["Total Spend"] > 0
                ? gender["Total PCV"] / gender["Total Spend"]
                : 0,
            MonthlyData: Array.from(gender.MonthlyData.values()).sort((a, b) =>
              a.Month.localeCompare(b.Month),
            ),
          }))
          .sort((a, b) => b["Total Spend"] - a["Total Spend"]);
    ...
    return res.status(500).json({
      success: false,
      message:
        "An error occurred while fetching Facebook Ad Account gender data",
      error: error.message,
    });
  }
};
...
const calculateBlendedDeviceSummary = (accountsData) => {
  ...
  const blendedSummary = Array.from(blendedData.values())
    .map((device) => ({
      ...device,
      "Total Purchase ROAS":
        device["Total Spend"] > 0
          ? device["Total PCV"] / device["Total Spend"]
          : 0,
...
export const fetchFbDeviceReports = async (req, res) => {
  ...
  try {
          ...
          .map((device) => ({
            ...
            "Total Purchase ROAS":
              device["Total Spend"] > 0
                ? device["Total PCV"] / device["Total Spend"]
                : 0,
            MonthlyData: Array.from(device.MonthlyData.values()).sort((a, b) =>
              a.Month.localeCompare(b.Month),
            ),
          }))
          .sort((a, b) => b["Total Spend"] - a["Total Spend"]);
    ...
    return res.status(500).json({
      success: false,
      message:
        "An error occurred while fetching Facebook Ad Account device data",
      error: error.message,
    });
  }
};
...
const calculateBlendedCountrySummary = (accountsData) => {
  ...
  const blendedSummary = Array.from(blendedData.values())
    .map((country) => ({
      ...country,
      "Total Purchase ROAS":
        country["Total Spend"] > 0
          ? country["Total PCV"] / country["Total Spend"]
          : 0,
...
export const fetchFbCountryReports = async (req, res) => {
  ...
  try {
          ...
          .map((country) => ({
            ...
            "Total Purchase ROAS":
              country["Total Spend"] > 0
                ? country["Total PCV"] / country["Total Spend"]
                : 0,
            MonthlyData: Array.from(country.MonthlyData.values()).sort((a, b) =>
              a.Month.localeCompare(b.Month),
            ),
          }))
          .sort((a, b) => b["Total Spend"] - a["Total Spend"]);
    ...
    return res.status(200).json({
      ...
      data: formattedResults,
      blendedCountryData: blendedSummary
        ? blendedSummary.blendedCountryData
        : [],
    });
  } catch (error) {
    ...
    return res.status(500).json({
      success: false,
      message:
        "An error occurred while fetching Facebook Ad Account country data",
      error: error.message,
    });
  }
};
...
const calculateBlendedAudienceSummary = (accountsData) => {
  ...
  accountsData.forEach((account) => {
    account.audienceData.forEach((audienceGroup) => {
      ...
      const currentAudienceData = blendedData.get(
        audienceGroup["Audience Segments"],
      );
  ...
  const blendedSummary = Array.from(blendedData.values())
    .map((audience) => ({
      ...audience,
      "Total Purchase ROAS":
        audience["Total Spend"] > 0
          ? audience["Total PCV"] / audience["Total Spend"]
          : 0,
...
export const fetchFbAudienceReports = async (req, res) => {
  ...
  try {
          ...
          .map((audience) => ({
            ...
            "Total Purchase ROAS":
              audience["Total Spend"] > 0
                ? audience["Total PCV"] / audience["Total Spend"]
                : 0,
            MonthlyData: Array.from(audience.MonthlyData.values()).sort(
    ...
    return res.status(200).json({
      ...
      data: formattedResults,
      blendedAudienceData: blendedSummary
        ? blendedSummary.blendedAudienceData
        : [],
    });
  } catch (error) {
    ...
    return res.status(500).json({
      success: false,
      message:
        "An error occurred while fetching Facebook Ad Account Audience data",
      error: error.message,
    });
  }
};
const calculateBlendedPlatformSummary = (accountsData) => {
  ...
  const blendedSummary = Array.from(blendedData.values())
    .map((platform) => ({
      ...platform,
      "Total Purchase ROAS":
        platform["Total Spend"] > 0
          ? platform["Total PCV"] / platform["Total Spend"]
          : 0,
...
export const fetchFbPlatformReports = async (req, res) => {
  ...
  try {
          ...
          .map((platform) => ({
            ...
            "Total Purchase ROAS":
              platform["Total Spend"] > 0
                ? platform["Total PCV"] / platform["Total Spend"]
                : 0,
            MonthlyData: Array.from(platform.MonthlyData.values()).sort(
              (a, b) => a.Month.localeCompare(b.Month),
            ),
          }))
          .sort((a, b) => b["Total Spend"] - a["Total Spend"]);
    ...
    return res.status(200).json({
      ...
      data: formattedResults,
      blendedPlatformData: blendedSummary
        ? blendedSummary.blendedPlatformData
        : [],
    });
  } catch (error) {
    ...
    return res.status(500).json({
      success: false,
      message:
        "An error occurred while fetching Facebook Ad Account platform data",
      error: error.message,
    });
  }
};
...
const calculateBlendedPlacementSummary = (accountsData) => {
  ...
  const blendedSummary = Array.from(blendedData.values())
    .map((placement) => ({
      ...placement,
      "Total Purchase ROAS":
        placement["Total Spend"] > 0
          ? placement["Total PCV"] / placement["Total Spend"]
          : 0,
...
export const fetchFbPlacementReports = async (req, res) => {
  ...
  try {
      ...
      try {
        do {
          ...
          for (const insight of response.data.data) {
            if (
              allData.size >= MAX_PLACEMENTS &&
              !allData.has(insight.platform_position)
            ) {
            ...
            .map((placement) => ({
              ...
              "Total Purchase ROAS":
                placement["Total Spend"] > 0
                  ? placement["Total PCV"] / placement["Total Spend"]
                  : 0,
              MonthlyData: Array.from(placement.MonthlyData.values()).sort(
                (a, b) => a.Month.localeCompare(b.Month),
              ),
            }));
    ...
    return res.status(200).json({
      ...
      data: formattedResults,
      blendedPlacementData: blendedSummary
        ? blendedSummary.blendedPlacementData
        : [],
    });
  } catch (error) {
    ...
    return res.status(500).json({
      success: false,
      message:
        "An error occurred while fetching Facebook Ad Account placement data",
      error: error.message,
    });
  }
};
...
const calculateBlendedAgeGenderSummary = (accountsData) => {
  const blendedData = new Map();
  accountsData.forEach((account) => {
    account.ageGenderData.forEach((comboGroup) => {
      if (!blendedData.has(comboGroup.age_gender)) {
        blendedData.set(comboGroup.age_gender, {
          age_gender: comboGroup.age_gender,
          "Total Spend": 0,
          "Total PCV": 0,
          MonthlyData: new Map(),
        });
      }
      const currentData = blendedData.get(comboGroup.age_gender);

      currentData["Total Spend"] += comboGroup["Total Spend"];
      currentData["Total PCV"] += comboGroup["Total PCV"];

      comboGroup.MonthlyData.forEach((month) => {
        const monthKey = month.Month;
        if (!currentData.MonthlyData.has(monthKey)) {
          currentData.MonthlyData.set(monthKey, {
            Month: monthKey,
            Spend: 0,
            "Purchase ROAS": 0,
            "Purchase Conversion Value": 0,
  ...
  return { blendedAgeGenderData: blendedSummary };
};

export const fetchFbAgeGenderReports = async (req, res) => {
  const { startDate, endDate } = req.body;
  const { brandId } = req.params;
    ...
    async function fetchTopAgeGender(accountId) {
      ...
      try {
        ...
        for (const insight of monthlyResponse.data.data) {
          ...
          comboData.MonthlyData.set(monthKey, monthData);
          comboData["Total Spend"] += spend;
          comboData["Total PCV"] += conversionValue;
        }
...
const calculateBlendedPlacementDeviceSummary = (accountsData) => {
  const blendedData = new Map();
  accountsData.forEach((account) => {
    account.placementDeviceData.forEach((comboGroup) => {
      if (!blendedData.has(comboGroup.placement_device)) {
        blendedData.set(comboGroup.placement_device, {
          placement_device: comboGroup.placement_device,
          "Total Spend": 0,
          "Total PCV": 0,
          MonthlyData: new Map(),
        });
      }
      const currentData = blendedData.get(comboGroup.placement_device);
  ...
  return { blendedPlacementDeviceData: blendedSummary };
};

export const fetchFbPlacementDeviceReports = async (req, res) => {
  const { startDate, endDate } = req.body;
  const { brandId } = req.params;
...
const calculateBlendedPlatformDeviceSummary = (accountsData) => {
  const blendedData = new Map();
  accountsData.forEach((account) => {
    account.platformDeviceData.forEach((comboGroup) => {
      if (!blendedData.has(comboGroup.platform_device)) {
        blendedData.set(comboGroup.platform_device, {
          platform_device: comboGroup.platform_device,
          "Total Spend": 0,
          "Total PCV": 0,
          MonthlyData: new Map(),
        });
      }
      const currentData = blendedData.get(comboGroup.platform_device);
  ...
  return { blendedPlatformDeviceData: blendedSummary };
};

export const fetchFbPlatformDeviceReports = async (req, res) => {
  const { startDate, endDate } = req.body;
  const { brandId } = req.params;
