...
export const setupHolidayGenerationCron = () => {
    ...
    async () => {
      ...
      try {
        ...
        for (const country of defaultCountries) {
          try {
            console.log(
              `📅 [Cron] Generating holidays for ${country} in ${currentYear}...`,
            );
            ...
            if (existingHolidays.length > 0) {
              console.log(
                `✅ [Cron] Holidays for ${country} in ${currentYear} already exist (${existingHolidays.length} holidays)`,
              );
              continue;
            }

            // Generate holidays using Calendarific API
            const holidays = await generateHolidaysWithAPI(
              country,
              currentYear,
            );
            ...
            console.log(
              `✅ [Cron] Successfully generated and stored ${insertedCount} holidays for ${country} in ${currentYear}`,
            );
          } catch (error) {
            console.error(
              `❌ [Cron] Error generating holidays for ${country}:`,
              error,
            );
        ...
        console.error(
          "❌ [Cron] Error in yearly holiday generation job:",
          error,
        );
      }
    },
  ...
  console.log(
    "✅ [Cron] Yearly holiday generation cron scheduled (January 1st, 2 AM UTC)",
  );
};
